...
# 2. Checking for presence of Probes
...
# 3. Verifying Secret usage
echo "Step 3: Verifying Secret usage..."
HAS_SECRET=$(kubectl get deployment ecommerce-app -o jsonpath='{.spec.template.spec.containers[0].envFrom[?(@.secretRef)]}')
